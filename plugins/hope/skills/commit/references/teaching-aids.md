# Commit Diff teaching aids

Read this reference only after `analysis.md` identifies a distinct teaching
job for a visual, microworld, or quiz. Use the smallest aid that performs that
job.
...
total combinations. Run `microworld-skeleton` as directed by `workflow.md`
and provide one grounded scenario for every returned combination.
