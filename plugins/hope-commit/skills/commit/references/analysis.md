...
- Compare commit subject and body claims with the changed-file map and code. A
...
- Write `title` as a short, direct statement of the observable change in the
  ...
  it is clearer than a full sentence. Do not copy the commit subject, lead
...
- Ground both `coreChange.before` and `coreChange.after` in collected
  changed-file code. The commit message may explain why the change exists, but
...
- A checked category needs a grounded basis and evidence whose role matches
  it. The commit message can establish stated intent; collected code is needed
  for code behavior.
